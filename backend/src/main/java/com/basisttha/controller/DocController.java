...
public class DocController {
    ...
    @PostMapping("/create")
    public ResponseEntity<DocumentDto> createDoc(
            @Valid @RequestBody DocTitleDto docTitleDto) {
        return new ResponseEntity<>(docService.createDoc(docTitleDto), HttpStatus.CREATED);
    }

    @DeleteMapping("/delete/{docId}")
    public ResponseEntity<Long> deleteDoc(
            @PathVariable Long docId)
            throws UnauthorizedUserException, ResourceNotFoundException {
        return ResponseEntity.ok(docService.deleteDoc(docId));
    }

    @PatchMapping("/rename/{docId}")
    public ResponseEntity<String> updateDocTitle(
            @PathVariable Long docId,
            @Valid @RequestBody DocTitleDto docTitleDto)
            throws UnauthorizedUserException, ResourceNotFoundException {
    ...
    public ResponseEntity<UserDocDto> addUser(
            @PathVariable Long docId,
            @RequestBody UserDocDto userDocDto)
            throws UnauthorizedUserException, ResourceNotFoundException, UserException {
        return ResponseEntity.ok(docService.addUser(docId, userDocDto));
    }

    @GetMapping("/users/shared/{docId}")
    public ResponseEntity<List<UserDocDto>> getSharedUsers(
            @PathVariable Long docId)
            throws UnauthorizedUserException, ResourceNotFoundException {
    ...
    public ResponseEntity<List<DocumentChangeDto>> getDocChanges(
            @PathVariable Long docId)
            throws UnauthorizedUserException, ResourceNotFoundException {
    ...
    @GetMapping("/{docId}")
    public ResponseEntity<DocumentDto> getDoc(
            @PathVariable Long docId)
            throws UnauthorizedUserException, ResourceNotFoundException {
    ...
    public ResponseEntity<Void> saveDoc(@PathVariable Long docId) {
        docService.saveDoc(docId);
